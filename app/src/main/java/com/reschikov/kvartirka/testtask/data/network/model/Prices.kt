...
data class Prices(private val day : Int,
    ...
    fun getPrice(context: Context) : String{
        ...
        if (day != 0) sb.append(context.getString(R.string.day)).append(day).append(LINE_BREAK)
        if (night != 0) sb.append(context.getString(R.string.night)).append(night).append(LINE_BREAK)
        if (hour != 0) sb.append(context.getString(R.string.hour)).append(hour)
        return sb.toString()
    }
}